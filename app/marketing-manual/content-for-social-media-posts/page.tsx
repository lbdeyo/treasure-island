...
export default async function ContentForSocialMediaPostsPage() {
  const textContent = await getPageTextContent(
    "marketing-content-for-social-media-posts",
  );

  const pageTitle = textContent?.title || "Content for Social Media Posts";
  const paragraphs = textContent?.paragraphs || [];

  return (
    <div className="bg-[url('/img/art/dangling.jpg')] bg-cover bg-top bg-repeat min-h-screen">
